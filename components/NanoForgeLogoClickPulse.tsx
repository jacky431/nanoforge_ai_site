...
type AnimationCtrl = ReturnType<typeof useAnimation>;
...
function Slice({
  ...
  i: number;
  breathControls: AnimationCtrl;
  triggerControls: AnimationCtrl;
  colorTarget: string;
}) {
  const baseRotate = i * 60 + 90; // 90도 추가하여 꼬리가 둘레를 따라가도록

  return (
    <motion.g
      transform={`rotate(${baseRotate} ${CX} ${CY})`}
      style={{ transformOrigin: `${CX}px ${CY}px` }}
      // 전역 트리거 변형(회전/스케일)은 바깥 그룹에 적용
      animate={triggerControls}
      initial={{ rotate: baseRotate, scale: 1 }}
    >
      {/* 기본 호흡은 내부 그룹에 적용 */}
      <motion.g animate={breathControls}>
        {/* 기본색 레이어 */}
        <motion.path d={SLICE_PATH} fill={baseColor} />
        {/* 트리거 시 색상 레이어 (opacity로 교차) */}
        <motion.path
          d={SLICE_PATH}
          fill={colorTarget}
          initial={{ opacity: 0 }}
          animate={{ opacity: [0, 0, 1, 0] }} // 트리거 구간에서 점등
...
export default function NanoForgeLogoClickPulse() {
  // 6조각 각각의 호흡 컨트롤러 - 각각 개별적으로 선언
  const breathCtrl0 = useAnimation();
  const breathCtrl1 = useAnimation();
  const breathCtrl2 = useAnimation();
  const breathCtrl3 = useAnimation();
  const breathCtrl4 = useAnimation();
  const breathCtrl5 = useAnimation();
  
  // 6조각에 동일하게 걸릴 전역 트리거 컨트롤러 - 각각 개별적으로 선언
  const triggerCtrl0 = useAnimation();
  const triggerCtrl1 = useAnimation();
  const triggerCtrl2 = useAnimation();
  const triggerCtrl3 = useAnimation();
  const triggerCtrl4 = useAnimation();
  const triggerCtrl5 = useAnimation();
  
  // 배열로 그룹화
  const breathCtrls = [breathCtrl0, breathCtrl1, breathCtrl2, breathCtrl3, breathCtrl4, breathCtrl5];
  const triggerCtrls = [triggerCtrl0, triggerCtrl1, triggerCtrl2, triggerCtrl3, triggerCtrl4, triggerCtrl5];
  ...
  useEffect(() => {
    ...
    return () => {
      breathCtrls.forEach((c) => c.stop());
      triggerCtrls.forEach((c) => c.stop());
    };
  }, [breathCtrls, triggerCtrls]);
  ...
  const runTrigger = async () => {
    ...
    await Promise.all(
      breathCtrls.map((c) =>
        c.start({ scale: 1, transition: { duration: 0.12 } })
      )
    );

    // 2) 전체 동시: 회전하며 0.1배로 수축 → 중앙으로 모임
    await Promise.all(
      triggerCtrls.map((c, idx) =>
        c.start({
          // 각 조각은 자기 base 회전값에 더해 한 바퀴 회전
          rotate: idx * 60 + 360,
          scale: 0.1,
    ...
    // 3) 색상 유지/전환한 상태로 회전하며 다시 커져 원래 자리 복귀
    await Promise.all(
      triggerCtrls.map((c, idx) =>
        c.start({
          rotate: idx * 60 + 720, // 복귀하며 추가 한 바퀴
          scale: 1,
    ...
    await Promise.all(
      triggerCtrls.map((c, idx) =>
        c.start({ rotate: idx * 60, scale: 1, transition: { duration: 0 } })
      )
    );
  ...
  return (
    <div className="w-full max-w-[180px] mx-auto select-none">
      {/* 클릭 anywhere to trigger */}
      <div onClick={runTrigger} className="cursor-pointer">
        <svg viewBox={`0 0 ${VB_W} ${VB_H}`} className="w-full h-auto">
          {[...Array(6)].map((_, i) => (
            <Slice
              key={i}
              i={i}
              breathControls={breathCtrls[i]}
              triggerControls={triggerCtrls[i]}
              colorTarget={altPalette[i]}
            />
          ))}
        </svg>
      </div>
